...
beforeEach(() => {
  jest.resetAllMocks();
  req = { params: {}, user: { id: 'u1', authority: 'student' }, file: undefined, files: undefined };
  res = {
...
describe('filesController', () => {
  describe('listFiles', () => {
    it('sends an array of S3 keys', async () => {
      ...
      expect(s3.listObjectsV2).toHaveBeenCalledWith({ Bucket: process.env.AWSS3_BUCKETNAME });
  ...
  describe('deleteFile', () => {
    it('removes submission references and deletes from S3', async () => {
      ...
      expect(s3.deleteObject).toHaveBeenCalledWith({ Bucket: process.env.AWSS3_BUCKETNAME, Key: 'file1.pdf' });
  ...
  describe('uploadRubric', () => {
    ...
    it('calls OpenAI parser and returns parsed rubric', async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ message: 'Rubric uploaded and parsed successfully', rubric: parsed });
    });
  });
});
