    ...
    try {
        ...
        if (functionCall && functionCall.arguments) {
            ...
            if (functionCall.name === 'createClassroom') {
                ...
                if (dueDate) {
                    try {
                        ...
                        return res.status(400).json({
                            ...
                            chatResponse: "I'm having trouble understanding the due date format. Could you please provide it in a format like '2024-03-20', or use phrases like 'next week', 'tomorrow', or 'in 3 days'?"
                        })
                    }
                }
                ...
                chatResponse = `I've created a new assignment called "${name}" in the "${classroomName}" classroom${parsedDueDate ? ` with a due date of ${parsedDueDate}` : ''}. Students can now view and submit their work for this assignment.`
            }
        } else {
        ...
        if (error.message.includes('JSON')) {
            ...
            return res
                ...
                .json({ 
                    ...
                    chatResponse: "I'm sorry, but I encountered an error processing your request. Could you please try rephrasing it?"
                })
        }
        res.status(500).json({ 
            error: error.message,
            chatResponse: "I apologize, but something went wrong while processing your request. Please try again or contact support if the problem persists."
        })
    }
}
...
const potential = async (req, res) => {
    try {
        ...
        const result = await grade(assignment.rubric, assignment.essay, gradingInstructions);
